<template>
  <div class="overflow-hidden rounded-xl border border-[#1e293b] bg-[#111827]">
    ...
    <!-- Column labels -->
    <div class="grid grid-cols-[auto_1fr_repeat(4,auto)] gap-x-4 border-t border-[#1e293b] bg-[#0f172a]/50 px-6 py-3">
      ...
      <span class="w-16 text-center text-[12px] font-semibold uppercase tracking-wider text-[#f1f5f9]">Win rate</span>
    </div>

    <!-- Rows -->
    <div
      v-for="row in rows" :key="row.pos"
      class="grid grid-cols-[auto_1fr_repeat(4,auto)] items-center gap-x-4 border-t border-[#1e293b]/60 px-6 py-3.5 transition hover:bg-[#1e293b]/40">
      ...
      <div class="flex min-w-0 items-center gap-3">
        <img v-if="row.logo" :src="row.logo" :alt="row.name" class="h-8 w-8 shrink-0 object-contain"
          @error="(e) => e.target.style.display='none'" />
        <span class="truncate text-[15px] font-medium text-[#f1f5f9]">{{ row.name }}</span>
      </div>
